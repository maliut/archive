...
public class Parser {
	...
	public void preParse() throws NotWellDefinedException {
		...
		while (matcher.find()) {
			...
			if (Symbol.isConnectiveNot(element)) {
				symbols.add(Symbol.createEmptyLetter(depth, symbols.size()));
			} else if (Symbol.isLeftParenthesis(element)) {
				depth++;
			} else if (Symbol.isRightParenthesis(element)) {
				depth--;
			}
	...
	private void parse(Node parent, int fromIndex, int toIndex) throws NotWellDefinedException {
		...
		if (ss.size() == 1 && ss.get(0) instanceof Letter) {
			Node me = new Node(getContentStr(ss), null);  // 生成对应的 node
			...
			// 2. 得到深度最小的连接词下标作为分割点
			Connective connective = (Connective) ss.stream().filter(s -> s instanceof Connective).min((s1, s2) -> (s1.depth - s2.depth)).get();
			int splitIndex = connective.sub;
			// 3. 构造 node
			Node me = new Node(getContentStr(ss), connective);  // 生成对应的 node
			if (root == null) root = me;  // 第一次设置根
			if (parent != null) parent.addChild(me);  // 连接父亲
			// 4. 处理左右的部分
	...
	private String getContentStr(List<Symbol> ss) {
		String str =  String.join(" ", ss.stream().map(s -> s.content).collect(Collectors.toList()));
		return str;
	}
